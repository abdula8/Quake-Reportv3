...
public final class QueryUtils {
    ...
    private QueryUtils() throws JSONException {
        ...
        JSONObject features  = reader.getJSONObject("features");
//        String place = features.getString("place");
//        String magnitude = features.getString("mag");
//        String time = features.getString("time");
    }
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for (int i = 0; i < earthquakeArray.length(); i++) {
                ...
                // Substring the string location into 2 strings in two textViews
                // let's get started...

                String[] parts = location.split(",");
                String locationOffset = parts[0]; // 004
                String primaryLocation = parts[1]; // 034556

                // 1- firstly we get the index of the comma to start from
//                int indexOfComma = location.indexOf(',');
//                int lengthOfLocation = location.length();
//                String locationOffset = location.substring(0, indexOfComma);
//                String primaryLocation = location.substring(indexOfComma, lengthOfLocation);
                Date dateObject = new Date(time);
                SimpleDateFormat dateFormat = new SimpleDateFormat("MMM DD, yyyy");
                String formattedDate = dateFormat.format(dateObject);
                Earthquake earthquake = new Earthquake(magnitude, locationOffset, primaryLocation, formattedDate);
